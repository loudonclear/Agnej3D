...
bool TriangleShape::pointWithinEdges(const glm::vec3 &point) {
    if (glm::dot(normal, glm::cross(vertices[1] - vertices[0], point - vertices[0])) < 0) return false;
    if (glm::dot(normal, glm::cross(vertices[2] - vertices[1], point - vertices[1])) < 0) return false;
    if (glm::dot(normal, glm::cross(vertices[0] - vertices[2], point - vertices[2])) < 0) return false;
